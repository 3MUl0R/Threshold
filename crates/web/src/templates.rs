...
pub fn build_template_env() -> Arc<minijinja::Environment<'static>> {
    ...
    env.add_template_owned(
        ...
        include_str!("../templates/error.html").to_string(),
    )
    .expect("error.html template");

    // Conversation templates
    env.add_template_owned(
        "conversations/list.html".to_string(),
        include_str!("../templates/conversations/list.html").to_string(),
    )
    .expect("conversations/list.html template");
    env.add_template_owned(
        "conversations/detail.html".to_string(),
        include_str!("../templates/conversations/detail.html").to_string(),
    )
    .expect("conversations/detail.html template");
    env.add_template_owned(
        "conversations/audit_partial.html".to_string(),
        include_str!("../templates/conversations/audit_partial.html").to_string(),
    )
    .expect("conversations/audit_partial.html template");

    // Schedule templates
    env.add_template_owned(
        "schedules/list.html".to_string(),
        include_str!("../templates/schedules/list.html").to_string(),
    )
    .expect("schedules/list.html template");

    // Audit templates
    env.add_template_owned(
        "audit/browser.html".to_string(),
        include_str!("../templates/audit/browser.html").to_string(),
    )
    .expect("audit/browser.html template");
    env.add_template_owned(
        "audit/tab_partial.html".to_string(),
        include_str!("../templates/audit/tab_partial.html").to_string(),
    )
    .expect("audit/tab_partial.html template");

    // Log templates
    env.add_template_owned(
        "logs/viewer.html".to_string(),
        include_str!("../templates/logs/viewer.html").to_string(),
    )
    .expect("logs/viewer.html template");
    env.add_template_owned(
        "logs/entries_partial.html".to_string(),
        include_str!("../templates/logs/entries_partial.html").to_string(),
    )
    .expect("logs/entries_partial.html template");

    // Config templates
    env.add_template_owned(
        "config/editor.html".to_string(),
        include_str!("../templates/config/editor.html").to_string(),
    )
    .expect("config/editor.html template");
    env.add_template_owned(
        "config/credentials.html".to_string(),
        include_str!("../templates/config/credentials.html").to_string(),
    )
...
fn tojson_filter(
    ...
    Ok(serde_json::to_string_pretty(&json_val).unwrap_or_else(|_| value.to_string()))
}
